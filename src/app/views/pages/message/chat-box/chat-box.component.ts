import { Component, OnInit, Input, HostListener, ElementRef, SimpleChanges, ChangeDetectorRef, ViewChild, Output, EventEmitter } from '@angular/core';
...
import { Conversation, StringList, ConversationType } from '../../../../core/model/general/message/conversation.model';
...
export class ChatBoxComponent implements OnInit {
  ...
  // chat container
  @ViewChild('chatContainer') chatContainer: PerfectScrollbarComponent;
  ...
  //select from cloud control
  cloudFileSelect = false;

  constructor(private messageService: MessageService, private userService: FireBaseUserService, private cdr: ChangeDetectorRef, public sanitizer: DomSanitizer, public iconService: IconService, public fireBaseUtility: FirebaseUtility, private pushNotification: PushNotificationService) { }

  ngOnInit() {
    this.innerWidth = window.innerWidth;
    let locationSubs = this.userService.getLocation().subscribe(
      location => {
        if (location.city) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.activeChat.currentValue.id) {
      if (changes.activeChat.currentValue.id != changes.activeChat.previousValue.id) {
        this.initChat();
        ...
        this.userLocations = {};
        this.messageTo = changes.activeChat.currentValue.messageTo;
  ...
  async loadChat(initialize = false) {
    try {
      ...
      let response = await this.messageService.getConversation(this._time, this._count.toString(), this.chatId).toPromise();
      ...
      if (response.code == 200 || response.code == 201) {
        ...
        if (initialize) {
          // console.log(data.conversation.filter(i=>i.type==3))
  ...
  formatChatData(initialize = false, response: Conversation[]) {
    ...
    data = data.filter(conversation => conversation.message);
    data.forEach(conversation => {
      this.processConversation(conversation);
    })

    data = data.filter(conversation => conversation.type == 3 ? conversation.attachment.crud != 3 : true);
    ...
    if (initialize) {
      ...
      this.cdr.detectChanges();
      this.chatContainer.directiveRef.scrollToElement('#msg-' + (data.length - 3));
    }
  ...
  formatAttachment(conversation: Conversation) {
    ...
    if (conversation.attachment.original && conversation.attachment.original.startsWith('http')) {
      conversation.attachment.original = conversation.attachment.original.replace('http://', 'https://');
    }

    if (conversation.attachment.media_type == 593) {
      conversation.attachment.youtube_embed = this.sanitizer.bypassSecurityTrustResourceUrl(`https://www.youtube.com/embed/${conversation.attachment.youtube_id}?enablejsapi=1`);
      conversation.attachment.youtube_link = this.sanitizer.bypassSecurityTrustUrl(`https://www.youtube.com/watch?v=${conversation.attachment.youtube_id}`);
    }
    else if (conversation.attachment.media_type == 587) {
      conversation.attachment.audio_link = new Audio(conversation.attachment.original);
      conversation.attachment.audio_link['audio_meta'] = {
        played: 0
      }

      conversation.attachment.audio_link.addEventListener('loadedmetadata', ($event) => {
        if (this.cdr) {
          this.cdr.detectChanges();
        }
        conversation.attachment.audio_link.removeEventListener('loadedmetadata', () => { });
      })
      ...
      if (conversation.attachment.hasOwnProperty('hls_3')) {
        if (conversation.attachment.hls_3.startsWith('http')) {
          conversation.attachment.hls_3 = conversation.attachment.hls_3.replace('http://', 'https://');
        }
  ...
  playAudio(audioElement: HTMLAudioElement) {
    ...
    audioElement['audio_meta']['update'] = setInterval(() => {
      audioElement['audio_meta']['played'] = Math.ceil((audioElement.currentTime / audioElement.duration) * 100);
  ...
  formatEvent(conversation: Conversation) {
    ...
    event['approxCost'] = (event.service_provider.rate / 100) * ((event['end'] - event['start']) / 60000);
    return event;
  }
  ...
  getChatObjectForText() {
    ...
    const chatFireBaseObj = {
      ...
      receiver: this.otherUser.user_id
    }
  ...
  async sendTextMessage() {
    try {
       ...
       */
      if (!this.userToImgMap.hasOwnProperty(chatObj.chatFireBaseObj.chat.sender.user_id)) {
      ...
      setTimeout(() => { this.chatContainer.directiveRef.scrollToBottom() }, 0)
      this.cdr.detectChanges();
      this.chatIdSet.add(chatObj.chatFireBaseObj.chat.message_id);
      await this.fireBaseUtility.addDataToCollection(this.messageSendFireStorePath, chatObj.chatFireBaseObj);
  ...
  async onMessage(data) {
     ...
     */
    if (conversation.topic.topic_id != this.topic.topic_id || this.chatIdSet.has(conversation.message_id)) return;
  ...
  async updateUserLocations(){
    if( this.currentUser ){
      if( !this.userLocations.hasOwnProperty(this.currentUser.user_id) ){
        let data = await this.fireBaseUtility.getDocument(this.userLocationsFireStorePath + this.currentUser.user_id).toPromise();
        if( data ){
    ...
    for( let users of this.otherUsers ){
      if( !this.userLocations.hasOwnProperty(users.user_id) ){
        let data = await this.fireBaseUtility.getDocument(this.userLocationsFireStorePath + users.user_id).toPromise();
        if( data ){
      ...
      if (currUserInfo.country == userInfo.country) {
        if (currUserInfo.state == userInfo.state) {
          if (currUserInfo.city == userInfo.city) {
            // same city
            return `now ${receiverTime.format('h:mm a')}, ${suffix}`;
          }
          // Same state
          return `${userInfo.city} now ${receiverTime.format('h:mm a')}, ${suffix}`
        }
        // Same country
        return `${userInfo.state} now ${receiverTime.format('h:mm a')}, ${suffix}`
      }
    }
    // Different country
    return `${userInfo.country} now ${receiverTime.format('h:mm a')}, ${suffix}`;
  }

  dragEnter(){
    console.log('Enter')
    this.displayDropArea++;
    // ev.stopPropagation();
  }
  ...
  async cloudSelectClosed(selected: any) {
    if (selected) {
      for (let selectedFile of selected) {
        ...
        chatObj.chatFireBaseObj.chat.message = JSON.stringify(selectedFile);
        chatObj.toChat.message = JSON.stringify(selectedFile);
        this.formatAttachment(chatObj.toChat);
        chatObj.chatFireBaseObj.chat.type = 3;
        chatObj.toChat.type = 3
        console.log(chatObj);
        await this.fireBaseUtility.addDataToCollection(this.messageSendFireStorePath, chatObj.chatFireBaseObj);
        this.chat.push(chatObj.toChat);
        this.chatIdSet.add(chatObj.toChat.message_id)


        let temp: any = {
          topic_id: this.topic.topic_id,
          message_id: chatObj.toChat.message_id,
          _time: this.toUnix(chatObj.toChat.time)
        };

        switch (chatObj.toChat.attachment.media_type) {
          case 577:
            temp.message = 'Photo';
            break;
          case 569:
            ...
            break;
          default:
            temp.message = chatObj.toChat.attachment.title;
            break;
        }
        this.messageSent.emit(temp)
        // scroll new message to view
