...
export class FileUploadAreaComponent implements OnInit {
  ...
  constructor(private userService: FireBaseUserService, private cdr: ChangeDetectorRef, private s3Service: AWSService, private _snackBar: MatSnackBar, private attachmentUtility: AttachmentUtility) { }
  ...
  async submit() {
    try {
      ...
      if (!file.uploadError) {
        ...
        this.openSnackBar(`File successfully uploaded`, {success: true});
  ...
  cancel() {
    ...
    this.fileSelectError = 'File upload canceled';
    this.newFiles = [];
    this.close(undefined);
  }
