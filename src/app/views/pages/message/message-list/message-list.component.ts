...
export class MessageListComponent implements OnInit {
    ...
    if(value){
      ...
      for( let chat of this._chatHistory ){
        ...
        if( chat.topic.topic_id == value.topic_id ){
          if( chat.chat._time <= value._time ){
  ...
  constructor(private messageService: MessageService, private cdr: ChangeDetectorRef, private pushNotification: PushNotificationService, private userService: FireBaseUserService) { }
  ...
  startListeningForMessages(){
    this.msgListener = this.pushNotification.Message.subscribe((newMsg) => this.onMessage(newMsg));
  }
  ...
  initMessage(_initialLoad) {
    ...
    this.messageService.getSnapshot(this._time, this._count.toString()).subscribe(
      response => {
        ...
        if (response['code'] == 200 || response['code'] == 201) {
          ...
          if (!this.initialLoad && _initialLoad) {
            ...
            if (this._chatHistory.length) {
              this._chatHistory[0].opened = true;
              this.chatToggled.emit({
                id: this._chatHistory[0].chat.topic_id,
                displayName: this._chatHistory[0].otherUser.name,
                messageTo: this._chatHistory[0].otherUser.name
              });
            }
            else {
  ...
  formaMessageListData(response) {
    ...
    if (response['data'].hasOwnProperty('snapshot')) {
      ...
      chatHistory.forEach((item, index) => {
        ...
        else if (item.chat.type == 2) chatHistory[index].chat.message = chatHistory[index].chat.message?JSON.parse(chatHistory[index].chat.message)['agenda']:'Agenda';

        this.msgIdSet.add(item.chat.message_id);
      })

      if (chatHistory.length) {
        this._time = chatHistory[chatHistory.length - 1].chat._time - 100;
      }
  ...
  processMessage(data: SnapShot[]) {
    if (this._chatHistory.length) {
      ...
      data.forEach((snap, idx) => {
        ...
        else { this.topicIDSet.add(snap.chat.topic_id); }
      })
      ...
      data.forEach(snap => this.topicIDSet.add(snap.chat.topic_id));
  ...
  toggleActive(idx) {
    ...
    this.chatToggled.emit({
      id: this._chatHistory[idx].chat.topic_id,
      displayName: this._chatHistory[idx].otherUser.name,
      messageTo: this._chatHistory[idx].otherUser.name
    });
    this.cdr.detectChanges();
  }
  ...
  ngOnDestroy(){
    if( this.msgListener ){
      ...
      this.pushNotification.reInitializeMessageObs();
    }
  }
